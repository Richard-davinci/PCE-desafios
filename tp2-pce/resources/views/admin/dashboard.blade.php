  ...
  <section class="mt-3 py-5 bg-gradient-dark text-light ">
    <div class="container d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-2">
      <div class="mb-4">
        ...
        <p class="text-blanco mb-2">Bienvenido al panel de administración. Aquí podrás gestionar todos los aspectos del
          sistema.</p>
      </div>
    </div>
  </section>
          ...
          <div class="card-body">
            ...
            <ul class="list-group list-group-flush">
              @foreach($lastUsers as $user)
                <li
                  class="list-group-item bg-azul text-light border-light d-flex align-items-center justify-content-between">
                  <span class="d-flex align-items-center">
                      <img src="{{ $user->profile_image ?? asset('img/default.webp') }}"
                           class="avatar-sm border border-secondary me-2 rounded-circle" alt="">
                      <span>{{ $user->name }}</span>
                  </span>
                  <div class="d-flex align-items-center">
                    @if($user->created_at->isToday())
                      <span class="badge text-bg-success me-2">Nuevo</span>
                    @else
          ...
          <div class="card-body">
            ...
            <div class="mb-2">
              <div class="d-flex justify-content-between">
                <span class="small">Administradores: {{ $rolesStats['admin']['count'] }}</span>
                <span class="small">{{ $rolesStats['admin']['percent'] }}%</span>
              </div>
              <div class="progress" style="height: 6px;">
                <div class="progress-bar bg-turquesa" style="width: {{ $rolesStats['admin']['percent'] }}%"></div>
              </div>
            </div>

            <div class="mb-2">
              <div class="d-flex justify-content-between">
                <span class="small">Usuarios: {{ $rolesStats['user']['count'] }}</span>
                <span class="small">{{ $rolesStats['user']['percent'] }}%</span>
              </div>
              <div class="progress" style="height: 6px;">
                <div class="progress-bar bg-turquesa" style="width: {{ $rolesStats['user']['percent'] }}%"></div>
              </div>
            </div>
          </div>
        </div>
            ...
            <div class="card-body">
              ...
              <ul class="list-group list-group-flush">
                <li class="list-group-item bg-azul text-light border-light d-flex justify-content-between">
                  <span><i
                      class="bi bi-person-plus me-2 text-turquesa"></i>Nuevo usuario: <strong>Ana Gómez</strong></span>
                  <span class="text-blanco small">hace 2 h</span>
                </li>
                <li class="list-group-item bg-azul text-light border-light d-flex justify-content-between">
                  <span><i
                      class="bi bi-bag-check me-2 text-turquesa"></i>Alta de servicio: <strong>Hosting + Mant.</strong></span>
